...
@section('title', 'Gestion des Activités - Gestion Kourel')
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center py-6">
                <div class="flex items-center">
                    <h1 class="text-3xl font-bold text-white"><i class="fas fa-calendar-alt mr-3"></i>Gestion des Activités</h1>
                </div>
            ...
            <div class="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 hover:bg-white/15 transition-all duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-white text-3xl font-bold mt-2">{{ $stats['total_activites'] }}</p>
            ...
            <div class="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 hover:bg-white/15 transition-all duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-white text-3xl font-bold mt-2">{{ $stats['activites_planifiees'] }}</p>
            ...
            <div class="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 hover:bg-white/15 transition-all duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-white text-3xl font-bold mt-2">{{ $stats['activites_en_cours'] }}</p>
            ...
            <div class="bg-white/10 backdrop-blur-xl rounded-2xl p-6 border border-white/20 hover:bg-white/15 transition-all duration-300">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p class="text-white text-3xl font-bold mt-2">{{ $stats['activites_terminees'] }}</p>
                        <p class="text-gray-400 text-sm mt-1">Cette semaine</p>
                    </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-white/20">
                    <thead class="bg-white/5">
                        <tr>
                            <th class="px-6 py-3 text-left text-xs font-medium text-white/70 uppercase tracking-wider">Activité</th>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-white/70 uppercase tracking-wider">Responsable</th>
                    ...
                    <tbody id="listeActivites" class="bg-white/5 divide-y divide-white/20">
                        @forelse($activites as $activite)
                        <tr class="hover:bg-white/10 transition-colors duration-200">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div>
                                    <div class="text-sm font-medium text-white">{{ $activite->nom }}</div>
                                    <div class="text-sm text-white/60">{{ Str::limit($activite->description, 50) }}</div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-white">
                                <div>{{ $activite->date_debut->format('d/m/Y') }}</div>
                                <div class="text-white/60">{{ $activite->date_debut->format('H:i') }} - {{ $activite->date_fin->format('H:i') }}</div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-white">
                                {{ $activite->lieu ?: 'Non spécifié' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-white">
                                {{ $activite->responsable ? $activite->responsable->nom . ' ' . $activite->responsable->prenom : 'Non assigné' }}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                <div class="flex items-center space-x-2">
                                    ...
                                    <a href="{{ route('activites.presences', $activite->id) }}" 
                                       class="text-green-400 hover:text-green-300 transition-colors duration-200"
                                       title="Gérer les présences">
                                        <i class="fas fa-users"></i>
                                    </a>
                                    <button onclick="supprimerActivite({{ $activite->id }}, '{{ $activite->nom }}')"
...
<script>
// Variables globales
let activites = @json($activites->items());
...
document.getElementById('formAjoutActivite').addEventListener('submit', function(e) {
    e.preventDefault();
...
function supprimerActivite(id, nom) {
    if (typeof alerteModerne !== 'undefined' && alerteModerne.confirmation) {
        alerteModerne.confirmation(
            `Êtes-vous sûr de vouloir supprimer l'activité "${nom}" ?`,
            function(confirmed) {
                if (confirmed) {
                    supprimerActiviteConfirm(id);
                }
            }
        );
    } else {
...
function supprimerActiviteConfirm(id) {
    fetch(`/activites/${id}`, {
        method: 'DELETE',
        headers: {
            'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
...
function reinitialiserFiltres() {
    document.getElementById('rechercheActivite').value = '';
...
function filtrerActivites() {
    const recherche = document.getElementById('rechercheActivite').value.toLowerCase();
    const type = document.getElementById('filtreType').value;
    const statut = document.getElementById('filtreStatut').value;
    
    const lignes = document.querySelectorAll('#listeActivites tr');
    
    lignes.forEach(ligne => {
        if (ligne.querySelector('td')) {
            const nom = ligne.querySelector('td').textContent.toLowerCase();
            const typeActivite = ligne.querySelector('td:nth-child(2)').textContent.toLowerCase();
            const statutActivite = ligne.querySelector('td:nth-child(6)').textContent.toLowerCase();
            
            const correspondRecherche = nom.includes(recherche);
            const correspondType = !type || typeActivite.includes(type);
            const correspondStatut = !statut || statutActivite.includes(statut);
...
// Event listeners
document.getElementById('rechercheActivite').addEventListener('input', filtrerActivites);
document.getElementById('filtreType').addEventListener('change', filtrerActivites);
document.getElementById('filtreStatut').addEventListener('change', filtrerActivites);
